...
export function AdminInventoryRequestsSection() {
  ...
  const submitReview = () => {
    ...
    const mutation = reviewDialog.mode === "approve" ? approveRequest : rejectRequest;
  ...
  const filteredItemSuggestions = useMemo(() => {
    ...
    return inventory.filter((item) => item.name.toLowerCase().includes(query)).slice(0, 8);
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Manage stock, review employee hardware requests, and track what has been assigned.
            </p>
          </div>
                ...
                <div className="relative">
                  <Input
                    ...
                    onChange={(event) => {
                      ...
                      if (editingItem && normalizedNextName !== normalizedEditingName) {
                        setEditingItem(null);
